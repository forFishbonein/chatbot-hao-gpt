  ...
  if (targetNode.nodeName === "DIV") {
    ...
    if (childrenNode[0].style.display === "none") {
      if (childrenNode[2].style.display === "none") {
        childrenNode[2].style.display = "block";
      }
      if (childrenNode[3].style.display === "none") {
        childrenNode[3].style.display = "block";
      }
    }
    ...
    siblings.forEach((e) => {
      if (e !== event.currentTarget) {
        ...
        if (child[2].style.display === "block") {
          child[2].style.display = "none";
        }
        if (child[3].style.display === "block") {
          child[3].style.display = "none";
        }
        if (child[4].style.display === "block") {
...
const initAll = () => {
  ...
  console.log(messages.value); // TODO:这里点击有时候会进入这个，还有样式！
...
const editTheTitle = (event, title) => {
  ...
  if (childrenNodes[0].style.display === "none") {
    childrenNodes[0].style.display = "block";
  }
  if (childrenNodes[1].style.display === "block") {
    childrenNodes[1].style.display = "none";
  }
  if (childrenNodes[2].style.display === "block") {
    childrenNodes[2].style.display = "none";
  }
  if (childrenNodes[3].style.display === "block") {
    childrenNodes[3].style.display = "none";
  }
  if (childrenNodes[4].style.display === "none") {
    childrenNodes[4].style.display = "block";
  }
};
...
const saveTheTitle = (event, id) => {
  ...
  // 获取该节点的兄弟节点
  const childrenNodes = parent3Node.children;
  if (childrenNodes[0].style.display === "block") {
    childrenNodes[0].style.display = "none";
  }
  if (childrenNodes[1].style.display === "none") {
    childrenNodes[1].style.display = "block";
  }
  if (childrenNodes[2].style.display === "none") {
    childrenNodes[2].style.display = "block";
  }
  if (childrenNodes[3].style.display === "none") {
    childrenNodes[3].style.display = "block";
  }
  if (childrenNodes[4].style.display === "block") {
    childrenNodes[4].style.display = "none";
  }
};
let requestingFlag = false;
let indexFlag = ref(0); // TODO 如果有初始消息，那么初始索引应该为0或者2
// let indexFlag = ref(1);
// let indexFlag = ref(2);
...
const outputText = (data) => {
  ...
  let timer = setInterval(type, 100); // 每隔100ms调用type函数
};
const clickToSend = () => {
  if (prompt.value) {
...
let resizeMainContainer = function () {
  ...
  // 添加事件监听器，以捕获鼠标滚轮事件
  // @ts-ignore
  messageWapper.addEventListener(
    "wheel",
    function (event) {
      // 阻止默认的滚轮缩放行为
  ...
  <div id="main-container">
    <div class="history-wapper">
      <el-scrollbar :max-height="maxHeight2">
        <ul class="history-ul">
          <li class="create-new" @click="createNewDialog">创建一个新对话</li>
          <li
            class="history-item"
          ...
          >
            ...
            <div style="display: none" @click.stop>
              <el-icon @click.stop><Delete @click.stop /></el-icon>
            </div>
            <div style="display: none" @click.stop>
              <el-icon @click.stop

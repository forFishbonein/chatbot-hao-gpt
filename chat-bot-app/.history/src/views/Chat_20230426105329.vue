...
const initMessageFlag = ref(false);
const initMessage = () => {
  initMessageFlag.value = true; //保证只能初始化一次
  messages.value.push({
    content: "加载中...",
    isBot: true,
  });
...
const initAll = () => {
  ...
  //   }
  // }, 200);
  if (messages.value.length === 0 && initMessageFlag.value === false) {
    //如果历史记录中没有内容
    initMessage(); //调用初始化文字的方法
  }
};
...
let indexFlag = ref(NaN); // TODO 如果有初始消息，那么初始索引应该为0或者2
// let indexFlag = ref(1);
// let indexFlag = ref(2);

...
const verifyHuman = () => {
  // 验证通过后，设置隐藏输入框的值为1
  const input = document.getElementById("human-verified");
  // @ts-ignore
  input.value = "1";
};
...
const initVerification = () => {
  ...
  window.addEventListener("beforeunload", (e) => {
    // 在关闭窗口时，如果隐藏输入框的值仍未被设置为1，则提示用户需要完成验证
    const input = document.getElementById("human-verified");
    // @ts-ignore
    if (input.value !== "1") {
      e.preventDefault();
...
window.addEventListener("DOMContentLoaded", initVerification);
onMounted(() => {
  //设置容器高宽
  resizeMainContainer();
  resizeHeight();
...
<template>
  ...
  <!-- HTML代码 -->
  <div>
    <button type="button" id="verification-button">点击此处以验证</button>
    <input type="hidden" name="human-verified" id="human-verified" />
  </div>
</template>
